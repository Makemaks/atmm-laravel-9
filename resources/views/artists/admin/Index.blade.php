...
@section('page_heading','Artists')
@section('section')
<admin-index-artist-component inline-template>
    <div class="col-sm-12">
        ...
        <div class="row">
            <div class="col-md-6">
                <searched :search="search" @gosearch="getArtistList('search')"></searched>
            </div>
            <div class="col-md-6 text-right">
            <a type="button" href="/artists/create" class="btn btn-primary" data-dismiss="modal">
        ...
        >
                ...
                <table class="table table-hover custom-table-css">
                    <thead>
                        <th>
                            ...
                            <a href="#" @click="sortArtistList('name')">
                                NAME
                    ...
                    <tbody v-if="allArtist.length > 0">
                        <tr v-for="artist,key in allArtist">
                            <td>@{{ artist.name }}</td>
                            <td>
                              <i  class="fa fa-music text-info"
                                  data-keyboard="false"
                                  data-backdrop='static'
                                  @click="getSongList(artist.id,'Add')"
                              ...
                              <i  class="fa fa-music text-info"
                                  data-keyboard="false"
                                  data-backdrop='static'
                                  @click="getSongList(artist.id,'Remove')"
                                  style="cursor: pointer;margin-left: 15px;"
                                  title="Remove Multiple Songs From This Artist"
                              > - </i>
                                <ul style="padding:0" v-if="artist.music_details.length > 0">
                                  <li v-for="song,ind in artist.music_details" style="list-style: none;">
                                    <i  class="fa fa-trash text-danger"
                                        data-keyboard="false"
                                        data-backdrop='static'
                                        style="cursor: pointer;"
                                        title="Remove this song"
                                        @click="showDeleteSongForArtist(artist.id, artist.name, song.id, song.title)"
                                    >
                                    </i>
                                    @{{song.title}}
                                  </li>
                            ...
                            <td>
                                <a :href="'/artists/' + artist.id + '/edit'" type="button" class="btn btn-success btn-circle">
            ...
            <loading :active.sync="isLoadingArtistList" :can-cancel="false" :is-full-page="false"></loading>
        </div>
        <div class="row">
            <div style="float:right">
                <pagination v-if="pagination.last_page > 1" :pagination="pagination" :offset="5" @paginate="getArtistList()"></pagination>
            </div>
        </div>
        ...
        <div class="modal" id="selectSongs" tabindex="-1" role="dialog">
            <div class="modal-dialog modal-dialog-centered" role="document">
                ...
                <form class="form-horizontal was-validated" @submit.prevent="processMultipleSong" >
                    <div class="modal-header">
                        <h4 class="modal-title">@{{task}} Songs - @{{artist_name}}</h4>
        ...
        <div class="modal" id="deleteSongForArtist" tabindex="-1" role="dialog" aria-labelledby="deleteSongForArtist" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" id="exampleModalLongTitle">
                    ...
                    <div class="modal-body">
                        Are you sure you want to remove the song <u>@{{song_title}}</u> from <u>@{{artist_name}}</u>?
                    </div>
                    <div class="modal-footer">
