                ...
                <div class="col-xl-10 col-lg-11 col-md-12">
                    <div class="row">
                      @foreach($products as $ind => $product)
                        @php $product_name = trim(strtolower(str_replace('Subscription', '', $product->product_name))); @endphp
                        ...
                        @if (isset($product->subscription_plans) && count($product->subscription_plans) > 0)
                          @foreach ($product->subscription_plans as $subscription_plan)
                                ...
                                @if ($product_name == 'monthly' && $subscription_plan->active == 1 && $subscription_plan->is_hide == 0)
                                  <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12" style="padding-bottom: 25px;">
                                      <div class="subscription">
                                          <span class="type">Monthly</span>
                                          <p class="price">
                                              @php
                                              //$product_price = $subscription_plan->plan_price - ($subscription_plan->plan_price * $subscription_plan->sales_tax);
                                              //$product_price = round($product_price,2);
                                              @endphp
                                              <span class="currency">$</span>{{$subscription_plan->product_price}}<span class="currency">/MO</span>
                                          </p>
                                          <!-- <p class="info">(+ tax)</p> -->
                                ...
                                @endif
                                @if ($product_name == 'yearly' && $subscription_plan->active == 1 && $subscription_plan->is_hide == 0)
                                  <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12" style="padding-bottom: 25px;">
                                      <div class="subscription">
                                          <span class="promo">
                                              SAVE 15%
                                          </span>
                                          <span class="type">Yearly</span>
                                          <p class="price">
                                              <span class="currency">$</span>{{ round(($subscription_plan->product_price)/12,2) }}<span class="currency">/MO</span>
                                          </p>
                                          <p class="info">(${{$subscription_plan->product_price}} annually)</p>
                                          <a href="{{ config('app.url')}}/subscription/yearly" role="button" class="btn btn-sm d-block mx-auto custom-primary" >SIGN UP </a>
                                      </div>
                                  </div>
                                @endif
